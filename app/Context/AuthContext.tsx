...
// const defaultState : StoreContextType = {
//   showCart : false,
//   setShowCart : () => {},
//   cartItems : [],
//   onAdd : () => {},
...
export const AppProvider = ({children} : {children : ReactNode}) => {
  ...
  const onRemove = (product : product) => {
   const foundItems = cartItems.find((item : product) => item._id === product._id)
   if(!foundItems) return;
    const newCartItems = cartItems.filter((item : product) => item._id !== product._id)

    setTotalPrice((prevTotalPrice) => prevTotalPrice - foundItems.price * foundItems.quantity)
